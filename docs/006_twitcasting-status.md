...
## 更新（PR #4 追補）: オフライン表示を YouTube 枠と統一

YouTube 枠（007）のオフライン表示と**トーン・構成を統一**した。左右（YouTube / ツイキャス）で
オフライン時の見た目・文言が揃い、CTA 先だけ各サービスへ向く形に。

### 変更点
- 非配信（`result.ok && !isLive`）時の見せ方を刷新:
  - 旧: 「現在オフラインです。最近の配信:」＋アーカイブ一覧＋「ツイキャスをすべて見る →」。
  - 新: 共通コンポーネント **`OfflineNotice`** で
    「**OFFLINE** バッジ＋**いまは配信していません**＋親しみコピー
    『普段はツイキャスで配信してます。よかったら遊びに来てね！』＋**『ツイキャスへ →』CTA**
    （`https://twitcasting.tv/hoshiorange`・新規タブ）」を**主役表示**。
  - 最近の配信（アーカイブ）は「**過去の配信**」見出しで**控えめに副次表示**（YouTube の「過去のアーカイブ」と同じ扱い）。
    過去配信が無くても OfflineNotice は表示される（旧実装は履歴ゼロだとフォールバックに落ちていた）。
- **配信中（is_live:true）の LIVE 主役表示・取得失敗/トークン未設定時のリンクフォールバックは従来どおり維持。**
- リード文を「ツイキャスでのライブ配信状況を表示。」→「ツイキャスでの配信はこちらから。」に調整。
- 共通化: 新規 `src/components/OfflineNotice/`（`OfflineNotice.tsx` / `.module.css`）を YouTube 枠と共有。
  ツイキャス側の重複していたオフライン誘導 UI/スタイル（`offlineNote` / `profileLink`）は削除。
- ダーク/ライト・reduced-motion・Latest 2 カラム維持。`npx tsc --noEmit` / `npm run build` 成功。

### 文言メモ
ツイキャス側の副次見出しは「**過去の配信**」のまま（ツイキャス＝配信）。
YouTube 側は配信メイン運用のため「**過去のアーカイブ**」に統一（007 参照）。サービス特性に合わせた使い分け。

...
- 認証: **アプリケーション単位の Basic 認証**で公開ユーザー情報を取得可能。
  - ヘッダ: `Authorization: Basic base64(ClientID:ClientSecret)` / `X-Api-Version: 2.0` / `Accept: application/json`
...
- [x] `src/lib/twitcasting.ts` を新規作成（Basic 認証・is_live 判定・current_live・movies 取得、ISR、フォールバック）
...
## 実装内容
### 環境変数（サーバー専用・NEXT_PUBLIC_ なし）
| 変数名 | 用途 |
|---|---|
| `TWITCASTING_CLIENT_ID` | アプリの ClientID（Basic 認証） |
| `TWITCASTING_CLIENT_SECRET` | アプリの ClientSecret（Basic 認証） |
...
## 次にやること（ユーザー側）
1. ツイキャスでアプリ登録（https://twitcasting.tv/developer.php）し ClientID / ClientSecret を取得。
2. Vercel の環境変数に `TWITCASTING_CLIENT_ID` / `TWITCASTING_CLIENT_SECRET` を登録（ローカル確認用に `.env.local` にも設定）。
3. 反映後、配信中はライブカード、オフライン時は最近の配信一覧が自動表示される。
